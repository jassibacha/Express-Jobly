...
router.get('/', async function (req, res, next) {
    ...
    // Conver tthe strings to integers
    if (query.minEmployees !== undefined) {
        query.minEmployees = parseInt(query.minEmployees);
        // Test if it's not a valid number
        if (isNaN(query.minEmployees)) {
            throw new BadRequestError('Min employees is not a valid number');
        }
    }
    if (query.maxEmployees !== undefined) {
        query.maxEmployees = parseInt(query.maxEmployees);
        if (isNaN(query.maxEmployees)) {
            throw new BadRequestError('Max employees is not a valid number');
        }
    }
